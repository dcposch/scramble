...
-----END PGP PUBLIC KEY BLOCK-----`,
	...
	DeleteNameResolution(user.Token, user.EmailHost)
	SaveUser(user)
...
func TestPublicKeysHandler(t *testing.T) {
	...
	if notaryRes.PubHash != tUser.PublicHash {
		log.Fatalf("Name resolution returned wrong pubHash for %s, got %s, expected %s",
			tUser.EmailAddress, notaryRes.PubHash, tUser.PublicHash)
	}
